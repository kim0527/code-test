 ...
 *          데이터 변경 로직이 public으로 공개되어, 외부에서 엔티티 필드의 값을 수정할 수 있는 가능성을 제공합니다.
...
public class Product {
     ...
     *          그리고 데이터 관점에서, 상품없이 카테고리가 존재할 수 있기 때문에 카테고리안에 상품이 존재하는 구조로 카테고리가 더 넓은 생명주기를 가진다고 생각합니다.
     *
     * 개선안 :
     *          1. 별도의 Category 도메인을 생성할 것을 권장드립니다.
     *             별도의 Category 도메인을 정의하여, 도메인 정보를 저장하고 이를 코드로써 관리하는 방식입니다.
     *             Category가 별도의 엔티티로써 독립적인 생명주기를 가지게 되며, 유효한 카테고리 괸리를 위한 유효성검사 로직등의 카테고리 관련 로직을 분리할 수 있게 됩니다.
     *
     *             [예시 코드]
     *             @Entity
     ...
     * 원인 :
     *        public 생성자를 통해 외부에서 new 키워드로 객체를 생성할 수 있는 가능성을 제공합니다.
     *        현재 String에 대한 유효성 검사가 없는 점에서 유요하지 않은 값을 가진 객체 생성까지 초래할 수 있습니다.
     * 개선안 :
