 ...
 *       현재는 엔티티의 연관관계 존재하지 않지만, 추후에 여러 엔티티가 추가되고 연관관계가 형성되었을 때 문제가 발생합니다.
 *       [지연 로딩]
 ...
 *       [순환 참조]
 *       Product 객체 안에 양방향 관계로 설정되있는 A 엔티티가 존재하는 경우, Product가 컨트롤러에서 반환되면 Jackson이 직렬화 하는 과정에서 양방향 관계로 인해 무한으로 순회하게 됩니다.
 *
